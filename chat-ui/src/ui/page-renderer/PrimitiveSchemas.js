...
export const SHARED_DEFINITIONS = {
  action: {
    ...
    properties: {
      ...
      href:               { type: 'string', description: 'navigate: an app route. submit/delete: one fixed /api/modules/{module_id}/{action_id}. Not carried by event or workflow actions.' },
      event_type:         { type: 'string', description: 'event actions only: the ui.* reaction to emit.' },
      modal_id:           { type: 'string', description: 'event actions targeting ui.modal.open/close: the id of a Modal declared on this page.' },
      workflow_id:        { type: 'string', description: 'workflow actions only: the runtime workflow registry id.' },
      context_variables:  { type: 'object', description: 'workflow actions only.' },
      payload:            { type: 'object' },
...
export const PRIMITIVE_SCHEMAS = {
  ...
  ResourceTable: {
    ...
    properties: {
      ...
      pagination_mode:    { type: 'string', enum: ['client'], default: 'client', description: 'Server paging is not supported here. Use DataTable for paged endpoints; resource filters/sorts are not server query inputs.' },
  ...
  DataTable: {
    ...
    properties: {
      ...
      data_key:     { type: 'string', description: 'Response path containing rows. Required in server mode.' },
      selection:    { type: 'string', enum: ['none', 'single', 'multi'], default: 'none' },
      pagination:   { type: 'boolean', default: true },
      pagination_mode: { type: 'string', enum: ['client', 'server'], default: 'client', description: 'Select server for paged list endpoints. Requires pagination=true, an explicit module api_endpoint, data_key, total_key, and page_size (1-100). Sends canonical page/page_size/search GET inputs; server owns literal search, stable order and total. No local filtering/slicing or interactive sorting in server mode.' },
      total_key:    { type: 'string', description: 'Required in server mode: response path containing a nonnegative integer count of all matching owned rows before pagination. Omit in client mode; never infer it from page length.' },
      page_size:    { type: 'integer', minimum: 1, default: 20, description: 'Positive integer. Explicit 1-100 is required for server mode.' },
      search:       { type: 'boolean', default: true },
      search_keys:  { type: 'array', items: { type: 'string' }, description: 'Client mode row keys searched within loaded rows only. Omitted/null uses all column keys; [] searches no fields. Server mode does not apply these keys or send them: the module must implement the requested literal search fields across its full owned dataset.' },
      actions:      { type: 'array', items: SHARED_DEFINITIONS.action },
      empty: {
        type: 'object',
        properties: {
