"""End-to-end fixture tests for the database intent runtime path.
...
def _write_bundle(root: Path, intent: dict[str, Any] | None = None) -> None:
    ...
    if intent is not None:
        ...
        (config / "database_intent.json").write_text(
            json.dumps(intent),
            encoding="utf-8",
        )
...
@pytest.mark.asyncio
async def test_e2e_app_loads_database_intent_from_bundle(tmp_path: Path) -> None:
    ...
    assert result.database_intent["app_id"] == "app_e2e"
...
async def test_e2e_entity_index_carries_collection_metadata(tmp_path: Path) -> None:
    ...
    result = await AppLoader.load(str(tmp_path))
    projects_entry = result.database_entities_by_key[("projects", "projects")]
    tasks_entry = result.database_entities_by_key[("tasks", "tasks")]
...
@pytest.mark.asyncio
async def test_e2e_missing_database_intent_is_allowed(tmp_path: Path) -> None:
...
async def test_e2e_apply_indexes_creates_all_declared_indexes(tmp_path: Path) -> None:
    ...
    applied = await apply_database_indexes(load_result.database_intent, persistence=context)
...
async def test_e2e_unique_flag_is_forwarded_to_create_index(tmp_path: Path) -> None:
    ...
    await apply_database_indexes(load_result.database_intent, persistence=context)
...
async def test_e2e_apply_indexes_is_idempotent(tmp_path: Path) -> None:
    ...
    second_count = await apply_database_indexes(load_result.database_intent, persistence=context)
...
async def test_e2e_full_load_then_apply_indexes(
    ...
    assert load_result.database_intent is not None, "database_intent must be populated"
    assert load_result.database_intent["version"] == "1"
    ...
    context, client = _make_context()
    applied_first = await apply_database_indexes(load_result.database_intent, persistence=context)
    assert applied_first == 4

    # --- Phase 3: idempotent second application ---
    applied_second = await apply_database_indexes(load_result.database_intent, persistence=context)
