"""
Hosted-pack generation smoke test.

Test scenario: "Build a simple creator dashboard app that lets users view wallet
balance and request payouts using the hosted wallet capability."

Four validation levels:
    A. Static checks  — hook injection, hosted context formatting, adapter guidance
...
Drift checks fail if the final generated artifact tree contains:
    ...
    - hosted_entitlements mutation adapter
"""
...
def _load_resolver():
    return _load_module(
        ...
        f"tests.resolve_hosted_pack_templates.smoke.{id({})}",
    )
...
def _load_hook():
    ...
    import hook_hosted_capabilities_context as m  # noqa: PLC0415
    return m
...
class TestHostedWalletContextInjection:
    ...
    def test_agents_yaml_no_hosted_business_logic_rule(self) -> None:
        source = _read("factory_app/workflows/AppGenerator/agents.yaml")
        assert "no hosted business logic" in source

    def test_agents_yaml_controller_agent_thin_client_rule(self) -> None:
        source = _read("factory_app/workflows/AppGenerator/agents.yaml")
        assert "thin Python client" in source

    def test_agents_yaml_no_modules_hosted_pack_path(self) -> None:
        source = _read("factory_app/workflows/AppGenerator/agents.yaml")
...
_CREATOR_DASHBOARD_HOSTED_PACK: dict[str, Any] = {
    ...
    "label": "Hosted Wallet",
    "summary": "Hosted wallet — view balance and request payouts.",
...
class TestCreatorDashboardBuildPlan:
    ...
    def test_wallet_pack_is_hosted_pack(self) -> None:
        assert _CREATOR_DASHBOARD_HOSTED_PACK["capability_source"] == "hosted_pack"

    def test_wallet_pack_surface_kind_is_external_integration(self) -> None:
        assert _CREATOR_DASHBOARD_HOSTED_PACK["surface_kind"] == "external_integration"

    def test_wallet_pack_implementation_mode_is_external_integration(self) -> None:
        assert _CREATOR_DASHBOARD_HOSTED_PACK["implementation_mode"] == "external_integration"
        ...
        for task in _CREATOR_DASHBOARD_BUILD_TASKS:
            if task["task_type"] == "module_contract":
                assert task["capability_pack_id"] != "wallet", (
                    "module_contract task must not exist for wallet (hosted_pack)"
                )
        ...
        for task in _CREATOR_DASHBOARD_BUILD_TASKS:
            for path in task.get("owned_paths") or []:
                assert not path.startswith("modules/wallet/"), (
                    f"Drift: hosted wallet module path in owned_paths: {path}"
                )
    ...
    def test_build_plan_validates_with_app_build_plan_tool(self) -> None:
        ...
        result = mod.app_build_plan(
            AppBuildPlan={
                **_MINIMAL_PLAN_BASE,
                "capability_packs": [_CREATOR_DASHBOARD_HOSTED_PACK],
    ...
    def test_wallet_adapter_content_has_no_hosted_module_import(
        ...
        for entry in template_files:
            import_lines = [ln.strip() for ln in entry["content"].splitlines()
                            if ln.strip().startswith(("import ", "from "))]
            hosted_imports = [ln for ln in import_lines if "app.modules.wallet" in ln]
            assert not hosted_imports, (
                f"Drift: hosted wallet module imported in adapter: {hosted_imports}"
            )
...
class TestRealWalletTemplateDriftGuards:
    ...
    def test_real_template_has_no_hosted_wallet_module_import(self) -> None:
        bad = [ln for ln in self.import_lines if "app.modules.wallet" in ln]
        assert not bad, f"Real wallet template imports hosted module: {bad}"
    ...
    def test_real_template_has_no_hosted_entitlements_mutation(self) -> None:
        ...
        assert "hosted_entitlements.backend" not in self.content
...
class TestWalletPageBinding:
    ...
    Full UI binding conventions for hosted adapter pages are a follow-up:
...
"""
    ...
    def test_wallet_page_does_not_bind_directly_to_hosted_pack(self) -> None:
        """Page schema must not bind directly to the hosted wallet pack id."""
        # The page must go through the façade module, not the hosted pack id
        import re
        direct_hosted_refs = re.findall(r"/api/modules/wallet/", self._WALLET_PAGE_YAML)
        assert not direct_hosted_refs, (
            "Page binds directly to hosted wallet pack — must use façade module wallet_dashboard"
        )
    ...
    def test_wallet_page_binding_uses_facade_not_direct_module_path(self) -> None:
        """Pages calling hosted wallet bind through the façade module, not hosted pack id."""
        assert "/api/modules/wallet_dashboard/" in self._WALLET_PAGE_YAML
...
class TestFacadeModuleConvention:
    ...
    def test_facade_module_task_is_not_hosted_pack(self) -> None:
        """Façade module task must be module_contract, NOT a hosted_pack entry."""
        facade_task = next(
            t for t in _CREATOR_DASHBOARD_BUILD_TASKS
    ...
    def test_pages_do_not_bind_directly_to_hosted_pack_id(self) -> None:
    ...
    def test_modules_wallet_path_absent_in_all_owned_paths(self) -> None:
        ...
        for task in _CREATOR_DASHBOARD_BUILD_TASKS:
            for path in task.get("owned_paths") or []:
                assert not path.startswith("modules/wallet/"), (
                    f"Drift: owned_paths contains modules/wallet/ — hosted pack must not be regenerated: {path}"
                )

    def test_wallet_adapter_client_still_generated_as_api_surface_task(self) -> None:
        """The hosted adapter client must still be generated as an api_surface task."""
        adapter_tasks = [
            t for t in _CREATOR_DASHBOARD_BUILD_TASKS
    ...
    def test_appschemaagent_facade_binding_guidance_present(self) -> None:
        ...
        assert "hosted_pack_id" in source

    def test_serviceagent_hosted_adapter_integration_rule_present(self) -> None:
