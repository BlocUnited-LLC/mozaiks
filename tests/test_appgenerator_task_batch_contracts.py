...
def test_appgenerator_handoffs_start_from_agents_not_pseudo_user() -> None:
    ...
    assert interview_rules[0]["condition"] == "${interview_complete} == True"
...
async def test_appgenerator_task_batch_dogfood_path_executes_and_assembles() -> None:
    ...
    class _FakeWorkerAgent:
        async def ask(self, message, **kwargs):
            ...
            task = variables["current_build_task"]
            seen_tasks.append(
                {
                    "task_id": variables["current_build_task_id"],
                    "task_run_mode": variables["task_run_mode"],
                    "task_type": variables["current_build_task_type"],
                    "message": message,
                }
            )
            return SimpleNamespace(
                body=json.dumps(
                    {
                        "agent_message": f"completed {task['task_id']}",
                        "code_files": [
                            {
                                "filename": path,
                                "content": f"# generated by {task['task_id']} for {path}",
                            }
    ...
    await execute_task_batches_for_trigger(
        ...
        agents={
            "ConfigMiddlewareAgent": _FakeWorkerAgent(),
            "AppSchemaAgent": _FakeWorkerAgent(),
        },
