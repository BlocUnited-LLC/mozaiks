...
def test_configure_event_loop_policy_uses_selector_on_windows(monkeypatch) -> None:
    ...
    monkeypatch.setattr("scripts.run_live_workflow_smoke.os.name", "nt")
    monkeypatch.setattr(
        "scripts.run_live_workflow_smoke.asyncio.WindowsSelectorEventLoopPolicy",
        _FakePolicy,
        raising=False,
    )
    monkeypatch.setattr(
        "scripts.run_live_workflow_smoke.asyncio.set_event_loop_policy",
