...
BuildRecordStatus = _artifact_models_mod.ArtifactLifecycleStatus
BuildRecordValidationStatus = _artifact_models_mod.ArtifactValidationStatus
...
BuildRecordStore = _artifact_store_mod.ArtifactStore
...
@pytest.mark.asyncio
async def test_invalidate_build_family_marks_records_stale() -> None:
    store = BuildRecordStore.__new__(BuildRecordStore)
    versions_coll = MagicMock()
    ...
    modified = await store.invalidate_build_family(
        app_id="app-1",
        build_family="app_bundle",
        build_key="primary",
...
@pytest.mark.asyncio
async def test_invalidate_build_record_refs_marks_only_targeted_known_records() -> None:
    store = BuildRecordStore.__new__(BuildRecordStore)
...
@pytest.mark.asyncio
async def test_accept_build_record_supersedes_prior_current_record() -> None:
    store = BuildRecordStore.__new__(BuildRecordStore)
    versions_coll = MagicMock()
    current_raw = {
        "_id": "av_child",
...
async def test_get_stale_build_families_returns_stale_without_current() -> None:
    """Families with a STALE record but no CURRENT record are returned."""
    store = BuildRecordStore.__new__(BuildRecordStore)
    versions_coll = MagicMock()
    ...
    # workflow_bundle has a CURRENT record, so it is not stale
    # design_docs has no CURRENT record, so it remains stale
...
async def test_get_current_build_record_refs_returns_highest_current_per_family() -> None:
    """Returns the highest-versioned CURRENT build_record_id for each family."""
    store = BuildRecordStore.__new__(BuildRecordStore)
    versions_coll = MagicMock()
    cursor = MagicMock()
    ...
    # confirm only CURRENT records were queried
...
async def test_get_stale_build_families_clears_when_all_rebuilt() -> None:
    """If every stale family has a CURRENT record, the list is empty."""
    store = BuildRecordStore.__new__(BuildRecordStore)
    versions_coll = MagicMock()
    versions_coll.distinct = AsyncMock(
        side_effect=[
