...
def test_deploy_target_spec_validates_generic_container() -> None:
    ...
    for path in ENV_EXAMPLE_PATHS:
        assert path in spec["artifact_outputs"]
...
def test_production_profile_emits_readiness_handoff_without_deploy_workflow() -> None:
    ...
    assert "env.example" not in artifacts
    assert "deployment.manifest.json" in artifacts
    assert ".github/workflows/readiness.yml" in artifacts
...
def test_env_example_contains_placeholders_not_secrets() -> None:
    ...
    env_examples = {path: result["artifacts"][path] for path in ENV_EXAMPLE_PATHS}

    for path, env_example in env_examples.items():
        ...
        assert "Generated by Mozaiks deployment contract" in env_example
        assert path.endswith(".example")


def test_generated_environment_examples_label_staging_and_production() -> None:
    result = generate_deployment_artifacts(
        app_id="demo_app",
        deployment_profile="production_container",
        include_dockerfiles=False,
    ...
    assert artifacts[".env.staging.example"].splitlines()[1] == "# Environment template: staging"
    assert artifacts[".env.production.example"].splitlines()[1] == "# Environment template: production"
    for path in ENV_EXAMPLE_PATHS:
        assert path in manifest["generated_files"]
        assert path in manifest["deploy_target_spec"]["artifact_outputs"]


def test_generated_provision_template_references_generated_environment_examples() -> None:
    provision_template = _read("factory_app/build_context/infra/templates/scripts/provision.sh")

    assert ".env.${ENVIRONMENT}.example" in provision_template
    assert ".env.${ENVIRONMENT}" in provision_template


def test_gitignore_allows_generated_environment_examples_but_blocks_real_env_files() -> None:
