...
def test_init_command_prompts_for_name_when_missing(monkeypatch, tmp_path) -> None:
    ...
    assert (target_dir / "platform" / "config" / "ai.json").exists()
    assert (target_dir / "platform" / "config" / "shell.json").exists()
    assert (target_dir / "brand" / "theme_config.json").exists()
...
def test_init_command_starter_scaffold_seeds_entry_workflow(tmp_path) -> None:
    ...
    assert (target_dir / "platform" / "workflows" / "HelloWorkflow" / "orchestrator.yaml").exists()
