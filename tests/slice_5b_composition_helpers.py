...
def composition_fixture() -> dict[str, object]:
    ...
    agents = yaml.safe_load(
        (ROOT / "factory_app/workflows/AppGenerator/agents.yaml").read_text(
            encoding="utf-8"
        )
    )
    ...
    reusable = one(PlanDisposition.PRESERVE_UNOWNED, "app_service_support/")
    removed = one(PlanDisposition.PRESERVE_UNOWNED, "app_backend_support/")
    agent = one(PlanDisposition.AGENT_AUTHOR, "app_data_contract/")
    rendered = one(PlanDisposition.RENDER, "app_ui_page_schema/")
    preserved = one(PlanDisposition.PRESERVE_UNOWNED, "workflow_config/")
    ...
    admission = resolve_assignment_admission(
        assignment,
        structured_output_configs={"AppGenerator": structured},
        workflow_agent_configs={"AppGenerator": agents},
    )
    data_contract = json.dumps(
        {"version": "1", "app_id": "slice-5b-golden", "surfaces": []},
        sort_keys=True,
    ) + "\n"
    result = build_assignment_artifact_result(
        ...
        structured_output={
            ...
            "agent_message": "deterministic offline fixture",
        },
        artifacts={"data/contract.json": data_contract},
        structured_output_configs={"AppGenerator": structured},
        validator_runner=lambda _validator, files: json.loads(
