...
def test_builder_artifact_store_round_trips_concept_and_build_plan() -> None:
...
def test_builder_artifact_store_records_workflow_exports_and_returns_latest() -> None:
    pm = _FakePersistenceManager()
    store = BuilderArtifactStore(pm=pm)
...
def test_builder_artifact_store_persists_database_migration_history() -> None:
    pm = _FakePersistenceManager()
    store = BuilderArtifactStore(pm=pm)
