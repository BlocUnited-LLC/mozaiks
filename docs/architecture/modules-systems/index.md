# Module Systems

Module system docs define deterministic app runtime units and build-time
capability pack planning.

Read these docs when changing module shape, module authoring rules, capability
pack ownership, or AppGenerator module planning.

| Doc | Scope |
| --- | --- |
| [Module System](module-system.md) | Canonical module shape, actions, capabilities, and contracts |
| [Module Authoring Patterns](module-authoring-patterns.md) | Practical authoring order and backend split |
| [Internal Messaging](internal-messaging.md) | Messages, support, social graph, notifications, and hosted extension boundaries |
| [AppGenerator Capability Planning](appgenerator-capability-planning.md) | AppBuildPlan capability grouping and ownership classes |
| [Managed Capability Packs](managed-capability-packs.md) | Generated app facades for hosted/operator-managed services |
| [Framework Capability Classification](framework-capability-classification.md) | Universal, framework, hosted, generated, and external capability classes |
