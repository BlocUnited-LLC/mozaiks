...
| G | Slice 5A/5B closed assignment dependency refs, validator refs, structured-output refs, and base revision identity. Runtime participant identity is deliberately absent and remains AG2-owned; the later context slice must add only typed entitlement inputs, not an agent ACL. | `mozaiksai/core/workflow/plan_assignment_compiler.py`; `assignment_admission.py` | resolved |
