...
- `WorkflowAuthor` may write only `platform/workflows/*`
...
## Why The Current `platform/` Bundle Matters

The current `platform/` directory in this repo is the flagship runtime-output
example.

It proves that the runtime already consumes:

- `platform/app.json`
- shell projections such as `platform/config/navigation_config.json`
- modules under `platform/modules/*`
- automation files under `platform/automations/*`
- workflows under `platform/workflows/*`

The builder should reason toward the canonical bundle families, but it also has
to understand the real transitional runtime output it is targeting today.
