...
Mozaiks should manage application context through one control-plane substrate for
...
App and code context should become a first-class Studio/control-plane substrate.
...
artifact summaries, and control-plane context selection to these contracts.

## AppContextVersion

`AppContextVersion` is a versioned Studio/control-plane view of an application's
...
The control plane should own `current_app_context_version_id` or equivalent
...
`ArtifactVersion`. This creates control-plane management state only: it does not
...
The control plane can now read the current `AppContextVersion` as advisory
...
`block_requires_human_override`, the control plane can now create an
...
The current control-plane contract can build a non-mutating context refresh plan
...
Control-plane planning helpers live in
...
Stale context should block risky refinements. A control-plane implementation may
