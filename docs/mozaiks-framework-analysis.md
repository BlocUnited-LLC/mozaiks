...
| **Refinement Engine** | Classifier, router, decision policy, and coding worker are separate implementations behind a single harness interface. | `mozaiksai/control_plane/implementations/` |
...
- Do not optimize refinement routing until real users are triggering it with real requests.
