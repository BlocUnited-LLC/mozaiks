...
For strict resume routing requirements and handoff templates, see [MFJ Strict Resume Contract](MFJ_STRICT_RESUME_CONTRACT.md).
...
| Field | Type | Required | Description |
|---|---|---|---|
| `resume_agent` | string | Yes | Final presenter/target agent after fan-in |
| `resume_entry_agent` | string | Yes | Router agent the runtime resumes at first |
| `aggregation_strategy` | string | Yes | How to combine child outputs. See Aggregation Strategies below. |
| `inject_as` | string | Yes | Context variable key where aggregated results are stored in the parent. Must start with `mfj_`. |
