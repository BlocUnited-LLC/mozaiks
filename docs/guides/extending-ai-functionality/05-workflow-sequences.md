...
App/product workspaces can consume the shipped factory registry without copying
it by adding `"extends": "mozaiks.default_workflow_registry"` to their
`extension_registry.json`. The loader merges registry lists by `id`, honors
`{ "id": "...", "remove": true }` tombstones, and resolves inherited factory
workflow folders from packaged `factory_app/workflows/`.
