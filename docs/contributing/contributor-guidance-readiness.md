...
- Refinement Engine and refinement: `control-plane-refinement-change`
...
| Refinement Engine/refinement | `control-plane-refinement-change` | add `factory-build-workflow-change` when `workflow_sequence` composition or `extension_registry.json` routing also changes |
