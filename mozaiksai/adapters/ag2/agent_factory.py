...
from .messages import extract_images_from_conversation
...
def _compose_prompt_sections(sections: Sequence[Dict[str, Any]] | Dict[str, Any]) -> str:
    """Build the system message string from structured prompt_sections.
    
    Supported input formats:
    1. Array of sections (standard): list of ``{id, heading, content}`` dicts
    2. Named-field dict: keys like ``role``, ``objective``, etc. each mapping to
       a ``{heading, content}`` dict — converted to array format internally.
    """
    parts: List[str] = []
...
async def create_agents(
    ...
    try:
        from .llm_validation import get_llm_config as _get_base_llm_config
    ...
    for agent_name, agent_config in agent_configs.items():
        # A2A remote agent: delegate to the A2A factory and skip local creation
        try:
            from mozaiksai.adapters.ag2.a2a import is_a2a_agent_config, create_a2a_agent

            if is_a2a_agent_config(agent_config):
                a2a_agent = create_a2a_agent(
                    agent_name,
                    agent_config,
                    context_variables=context_variables,
                )
                agents[agent_name] = a2a_agent
                logger.info(f"[AGENTS] Created A2A remote agent '{agent_name}'")
                continue
        except ImportError as a2a_imp_err:
            logger.error(f"[AGENTS] A2A agent '{agent_name}' requested but AG2 A2A extras not installed: {a2a_imp_err}")
            raise
        except Exception as a2a_err:
            logger.error(f"[AGENTS] Failed creating A2A agent '{agent_name}': {a2a_err}")
            raise

        try:
            from .outputs import get_llm_for_workflow as _get_structured_llm
        ...
        # Build system_message from prompt_sections (the standard format)
        ...
        if prompt_sections:
            ...
            logger.warning(
                f"[AGENTS] Agent '{agent_name}' in workflow '{workflow_name}' has no prompt_sections — "
                f"using default system_message. All agents should define prompt_sections."
            )
        ...
        # Load update_agent_state hooks from hooks config for this agent
        # CRITICAL: These must be added BEFORE agent construction to work with AG2's update_agent_state_before_reply
        try:
            from mozaiksai.engine.execution.hooks import _resolve_import
            from pathlib import Path

            hooks_section = workflow_config.get('hooks', {})
            hooks_entries: list = []
            if isinstance(hooks_section, dict):
                hooks_entries = hooks_section.get('hooks') or hooks_section.get('hook_rules') or []
            elif isinstance(hooks_section, list):
                hooks_entries = hooks_section

            workflow_dir = workflow_manager.workflows_base_path / workflow_name

            for entry in hooks_entries:
                if (isinstance(entry, dict) and
                    entry.get("hook_type") == "update_agent_state" and
                    entry.get("hook_agent") in (agent_name, "all")):
                    
                    file_value = entry.get("filename")
                    fn_value = entry.get("function")
                    
                    if file_value and fn_value:
                        fn, qual = _resolve_import(workflow_name, file_value, fn_value, workflow_dir)
                        if fn:
        ...
        if agent_config.get("image_generation_enabled", False):
            ...
            try:
                ...
                from .llm_validation import get_dalle_llm_config
    ...
    try:
        from mozaiksai.kernel.workflow_manager import get_workflow_manager
