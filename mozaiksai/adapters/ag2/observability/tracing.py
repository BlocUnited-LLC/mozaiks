...
def _load_ag2_otel_module():
    """Load AG2 OpenTelemetry module, preferring latest namespace."""
    module_candidates = ("autogen.opentelemetry", "autogen.otel")
...
def instrument_llm_globally() -> bool:
    ...
    try:
        ag2_otel = _load_ag2_otel_module()
        if ag2_otel is None:
            logger.warning("AG2 OpenTelemetry module not available. Install with: pip install 'ag2[tracing]'")
            return False
        instrument_llm_wrapper = getattr(ag2_otel, "instrument_llm_wrapper", None)
        if not callable(instrument_llm_wrapper):
            logger.warning("AG2 OpenTelemetry module does not expose instrument_llm_wrapper")
            return False
...
def instrument_agent(agent: "ConversableAgent") -> bool:
    ...
    try:
        ...
        if ag2_otel is None:
            return False
        ag2_instrument_agent = getattr(ag2_otel, "instrument_agent", None)
        if not callable(ag2_instrument_agent):
            return False
...
def instrument_pattern(pattern: Any, pattern_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if ag2_otel is None:
            return False
        ag2_instrument_pattern = getattr(ag2_otel, "instrument_pattern", None)
        if not callable(ag2_instrument_pattern):
            return False
