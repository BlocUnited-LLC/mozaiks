# ============================================================================
# FILE: engine/agents/tools.py
# DESCRIPTION:
#   Agent tool function loading from workflow YAML config (tools section).
...
#   - Any other workflow-specific data from the YAML config
#   
...
def _wrap_with_validation(
    *,
    ...
    from mozaiksai.engine.validation.tools import validate_tool_call
    import time
...
def load_agent_tool_functions(workflow_name: str) -> Dict[str, List[Callable]]:
    ...
    Reads the tools section from the workflow YAML config and returns a mapping
    of agent_name -> list[callable] so callers can pass functions=... to
    ...
    if not isinstance(entries, list):
        logger.warning(f"[TOOLS] 'tools' section not a list in '{workflow_name}'")
        return mapping
