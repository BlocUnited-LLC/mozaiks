# ==============================================================================
# FILE: engine/execution/lifecycle.py
...
Purpose:
- Execute tools at orchestration boundaries (before_chat, after_chat, before_agent, after_agent, on_context_change)
- Driven by workflows/<workflow>/tools.yaml "lifecycle_tools" array
...
Schema (tools.yaml):
{
  "lifecycle_tools": [
    {
      "trigger": "before_chat",       # Required: before_chat | after_chat | before_agent | after_agent | on_context_change
      "agent": null,                  # Optional: Agent name for agent-level triggers, null for chat-level
...
class LifecycleToolManager:
    ...
    def __init__(self, workflow_name: str):
        ...
        self.tools: Dict[LifecycleTrigger, List[LifecycleTool]] = {
            ...
            LifecycleTrigger.ON_CONTEXT_CHANGE: [],
        }
        self._loaded = False
    ...
    async def execute_trigger(
        ...
        **kwargs: Any,
        ...
        """Dispatch a lifecycle trigger to the appropriate handler method.

        Accepts either a ``LifecycleTrigger`` enum or a raw string.
        Extra *kwargs* are forwarded to the underlying handler.
        """
        ...
        if resolved_trigger is LifecycleTrigger.ON_CONTEXT_CHANGE:
        ...
        logger.debug(f"[LIFECYCLE] Trigger '{resolved_trigger.value}' has no dispatch handler")
