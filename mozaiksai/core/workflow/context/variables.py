...
def _load_workflow_plan(workflow_name: str) -> Tuple[ContextVariablesPlan, Dict[str, Any]]:
    ...
    plan = _synthesize_mfj_context(workflow_name, plan)
    return plan, raw_section


# Fixed MFJ runtime resume handshake variables — synthesized automatically for
# any workflow whose pack graph declares mid_flight_journeys.
_MFJ_RESUME_FIELDS: List[Dict[str, Any]] = [
    {"name": "_mfj_resume_pending",        "type": "boolean", "default": False},
    {"name": "_mfj_resume_target_agent",   "type": "string",  "default": None},
    {"name": "_mfj_resume_entry_agent",    "type": "string",  "default": None},
    {"name": "_mfj_resume_nonce",          "type": "string",  "default": None},
    {"name": "_mfj_resume_consumed_nonce", "type": "string",  "default": None},
]


def _synthesize_mfj_context(workflow_name: str, plan: ContextVariablesPlan) -> ContextVariablesPlan:
    """Auto-register inject_as keys and _mfj_resume_* fields from the workflow's pack
    graph so workflow authors don't need to manually declare them in context_variables.yaml.

    Rules:
    - Explicit declarations in context_variables.yaml always win (never overwritten).
    - Each inject_as key gets: type=object, state, default=null, scoped to resume_agent.
    - Each resume_agent gets all five _mfj_resume_* fields in its scope.
    - If no mid_flight_journeys exist, this is a no-op.
    """
    try:
        from ..pack.graph import get_workflow_pack_graph
        graph = get_workflow_pack_graph(workflow_name)
    except Exception:
        return plan

    if not graph or not graph.mid_flight_journeys:
        return plan

    # Build: resume_agent → [inject_as keys] from expanded (flat) journeys.
    agent_inject_keys: Dict[str, List[str]] = {}
    for journey in graph.mid_flight_journeys:
        agent = journey.fan_in.resume_agent
        key = journey.fan_in.inject_as
        agent_inject_keys.setdefault(agent, [])
        if key not in agent_inject_keys[agent]:
            agent_inject_keys[agent].append(key)

    if not agent_inject_keys:
        return plan

    defs = dict(plan.definitions)
    agents = {name: ContextAgentView(variables=list(view.variables)) for name, view in plan.agents.items()}

    def _state_def(dtype: str, default: Any) -> ContextVariableDefinition:
        return ContextVariableDefinition(
            type=dtype,
            source=ContextVariableSource(type="state", default=default),
        )

    def _ensure_agent_has(agent_name: str, var_names: List[str]) -> None:
        view = agents.setdefault(agent_name, ContextAgentView())
        existing = set(view.variables)
        for v in var_names:
            if v not in existing:
                view.variables.append(v)
                existing.add(v)

    # Synthesize inject_as keys (object, null default).
    for agent_name, inject_keys in agent_inject_keys.items():
        for key in inject_keys:
            if key not in defs:
                defs[key] = _state_def("object", None)
        _ensure_agent_has(agent_name, inject_keys)

    # Synthesize the five _mfj_resume_* fields for every resume agent.
    resume_field_names = [f["name"] for f in _MFJ_RESUME_FIELDS]
    for field in _MFJ_RESUME_FIELDS:
        if field["name"] not in defs:
            defs[field["name"]] = _state_def(field["type"], field["default"])
    for agent_name in agent_inject_keys:
        _ensure_agent_has(agent_name, resume_field_names)

    return ContextVariablesPlan(definitions=defs, agents=agents)
