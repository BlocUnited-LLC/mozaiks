...
* **AgentSignalDecomposition** — triggered when an agent emits a
  ...
  containing a ``decomposition_request``).  Dynamic — decomposition happens
...
class AgentSignalDecomposition:
    ...
    2. A structured output containing a ``decomposition_request`` dict
       with ``is_multi_workflow: true`` and a ``workflows`` list.
    """
    ...
    def detect(self, context: DecompositionContext) -> DecompositionPlan | None:
        ...
        # Path 2: Structured output with decomposition_request
        structured = evt.get("structured_data") or evt.get("payload") or {}
        dr = structured.get("decomposition_request")
        if isinstance(dr, dict) and dr.get("is_multi_workflow"):
            return self._from_structured_output(dr, context)
    ...
    def _from_structured_output(self, dr: dict, ctx: DecompositionContext) -> DecompositionPlan | None:
        workflows = dr.get("workflows")
        ...
        return DecompositionPlan(
            sub_tasks=tuple(sub_tasks),
            execution_mode=ExecutionMode.PARALLEL,
            reason="agent structured output",
            resume_agent=_str_or_none(dr.get("resume_agent")),
            strategy_metadata={"decomposition_reason": dr.get("decomposition_reason")},
        )
