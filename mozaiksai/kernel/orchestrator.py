...
#
# It controls GroupChats — pause, decompose, spawn parallel sub-tasks
# via DAGExecutor, merge results, and resume the parent.
# ==============================================================================
"""
...
3. **Decomposition path**:
   ...
   - If a plan is returned:
     ...
     b. Converts sub-tasks to ``DAGTask``s and executes via ``DAGExecutor``
...
Decomposition dispatches through ``DAGExecutor``, which in turn uses
``RunSupervisor.start_run()`` for each task. The orchestrator itself
only speaks ``OrchestrationPort``.
"""
...
from mozaiksai.runtime.dag_executor import DAGExecutor, DAGTask
...
class UniversalOrchestrator:
    ...
    def _get_supervisor(self):
        """Return the injected supervisor or the global singleton."""
    ...
    async def resume(self, request: ResumeRequest) -> AsyncIterator[DomainEvent]:
        ...
        async for event in self._get_supervisor().resume_run(request):
            yield event
    ...
    async def cancel(self, run_id: str) -> None:
        ...
        await self._get_supervisor().cancel_run(run_id)
    ...
    async def _execute_single(
        ...
        """Dispatch through RunSupervisor for a normal single-capability run."""
        async for event in self._get_supervisor().start_run(request):
            yield event
    ...
    async def _execute_decomposition(
        ...
        """Execute a full decompose → DAG → merge → (optional resume) cycle."""
        ...
        # 2. Convert sub-tasks → DAGTasks and execute via DAGExecutor
        dag_tasks = [
            DAGTask(
                task_id=st.task_id,
                capability="agent",
                ...
                input={
                    "initial_message": st.initial_message or "",
                    "parent_run_id": run.run_id,
                },
                metadata=st.metadata or {},
                depends_on=tuple(st.depends_on) if st.depends_on else (),
        ...
        executor = DAGExecutor(run_supervisor=self._get_supervisor())
        ...
        async for event in executor.execute_stream(
            ...
            yield event
            # Track child run IDs from task_started events
            if event.event_type == "dag.task_started":
                child_id = event.data.get("task_id")
                if child_id:
                    run.child_run_ids.append(child_id)

        # 3. Convert DAGResult → ChildResults for merge
        run.state = RunState.MERGING
        dag_result = executor.last_result

        child_results: list[ChildResult] = []
        if dag_result:
            for task_id, tr in dag_result.task_results.items():
                child_results.append(ChildResult(
                    task_id=tr.task_id,
                    workflow_name=next(
                        (st.workflow_name for st in plan.sub_tasks if st.task_id == tr.task_id),
                        tr.task_id,
                    ),
                    run_id=tr.run_id,
                    text_output=tr.text_output,
        ...
        merge_ctx = MergeContext(
            ...
            parent_context_variables=request.context,
    ...
    async def _resume_parent_with_merge(
        ...
        async for event in self._get_supervisor().resume_run(resume_request):
            yield event
