# ==============================================================================
# FILE: core/workflow/workflow_manager.py
...
class UnifiedWorkflowManager:
    ...
    def _load_workflow_tools(self, workflow_path: str) -> None:
        ...
        wp = _P(workflow_path)
        # Check config/ subdir first, then root (consistent with _load_config_if_exists)
        tools_yaml_path = wp / "config" / "tools.yaml"
        if not tools_yaml_path.exists():
            tools_yaml_path = wp / "tools.yaml"
        if not tools_yaml_path.exists():
            return
        ...
        try:
            ...
            logger.error(f"Failed parsing tools config for {workflow_name}: {e}")


    def get_ui_tool_record(self, tool_path_or_id: str) -> Optional[Dict[str, Any]]:
        """Lookup UI tool record by module path or tool id.

        Accepts either the full python path (used in the tools config) or the tool_id.
        """
    ...
    def discover_workflows(self) -> List[str]:
        ...
        for item in self.workflows_base_path.iterdir():
            if item.is_dir() and not item.name.startswith('.'):
                # Check for modular YAML config (config/ subdir or root)
                has_orchestrator = (
                    (item / "config" / "orchestrator.yaml").exists()
                    or (item / "orchestrator.yaml").exists()
                )
                if has_orchestrator:
                    workflows.append(item.name)
                    logger.debug(f"Discovered workflow (orchestrator.yaml): {item.name}")
                elif (item / "config" / "tools.yaml").exists() or (item / "tools.yaml").exists() or (item / "tools").exists():
                    workflows.append(item.name)
    ...
    def _load_single_workflow(self, workflow_name: str) -> WorkflowInfo:
        ...
        # Load configuration from modular YAML files
    ...
    def get_ui_hidden_triggers(self, workflow_name: str) -> Dict[str, Set[str]]:
        ...
        Extracts state variables with ui_hidden: true from the context_variables config
    ...
    def get_structured_output_registry(self, workflow_name: str) -> Dict[str, Optional[str]]:
        ...
        Supports both the dict-based registry ({"Agent": "Model"|None})
    ...
    def validate_workflow(self, workflow_name: str) -> Dict[str, Any]:
        ...
        # Check for modern modular structure (config/ subdir or root)
        orchestrator_yaml = self._resolve_config_path(workflow_path, "orchestrator")
        
        if orchestrator_yaml and orchestrator_yaml.exists():
        ...
        for config_name in config_names:
            resolved = self._resolve_config_path(workflow_path, config_name)
            if resolved:
    ...
    # ========================================================================
    def _resolve_config_path(self, base_path: Path, config_name: str) -> Optional[Path]:
        """Resolve the path to a YAML config file for a workflow.

        Checks ``config/`` subdirectory first, then workflow root.
        Returns the first existing path, or ``None``.
        """
        for candidate in (
            base_path / "config" / f"{config_name}.yaml",
    ...
    def _load_config_if_exists(self, base_path: Path, config_name: str) -> Dict[str, Any]:
        ...
        Supports both workflow layouts:
        - ``config/`` subdirectory (standard multi-file layout)
        - Workflow root (simple single-directory layout)
        
        Args:
            base_path: Workflow root directory
            config_name: Base name without extension (e.g., 'orchestrator', 'agents')
        
        Returns:
            Parsed config dict, or empty dict if file doesn't exist
        """
        yaml_path = self._resolve_config_path(base_path, config_name)
    ...
    def _load_modular_workflow_config(self, workflow_path: Path) -> Dict[str, Any]:
        ...
        Tools file expected to expose unified 'tools' list.
        ...
        if tools_data:
            # Validate: ensure only 'tools' key we care about; ignore unknown keys if present
    ...
    # ========================================================================
    # WorkflowRegistry PROTOCOL (mozaiksai.ports.registry)
    # ========================================================================
    ...
    # ========================================================================
    # RUNTIME HANDLER REGISTRY (decorator API)
    # ========================================================================
    ...
    def register_hooks(self, workflow_name: str, agents: Dict[str, Any], force: bool = False) -> List[Any]:
        """Register hooks for a workflow using the hooks config.
        ...
        try:
            from mozaiksai.engine.execution.hooks import register_hooks_for_workflow
            hooks_data = self._config_cache.get(workflow_name, {}).get('hooks')
