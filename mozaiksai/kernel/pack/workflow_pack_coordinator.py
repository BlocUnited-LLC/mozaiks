    ...
    if isinstance(mfj, list) and mfj:
        ...
        for item in mfj:
            if isinstance(item, dict):
                ...
                try:
                    from mozaiksai.kernel.pack.schema import _mfj_to_v2_dict
...
class MergeMode(str, Enum):
    ...
    COLLECT_ALL = "collect_all"  # raw child_extra dump
    ...
    * Supports configurable timeout and partial-failure strategies.
    ...
    def __init__(
        ...
        *,
        ...
        # Implements PackTransportPort (ports/pack_transport.py).
        # Wired by dispatcher.py at construction time.  None in legacy
        # test harnesses — guarded at each call site.
    ...
    def set_transport(self, transport: Any) -> None:
        """Late-bind the transport port (PackTransportPort).

        Called once during startup after the transport layer is available.
        """
    ...
    async def handle_structured_output_ready(self, event: Dict[str, Any]) -> None:
        ...
        # Resolve trigger list from v3 (mid_flight_journeys), v2 (journeys). Returns flat dicts in all cases.
        ...
        # Fallback: if strategy doesn't detect, no decomposition.
        ...
        if spawn_mode not in ("workflow", "shared_workflow"):
            spawn_mode = "workflow"

        shared_workflow = None
        if spawn_mode == "shared_workflow":
            shared_workflow = str(trigger_entry.get("shared_workflow") or "").strip()
            if not shared_workflow:
                logger.warning(
                    "[PACK] spawn_mode=shared_workflow requires trigger_entry.shared_workflow (parent=%s)",
                    parent_workflow,
                )
                return

        if not self._transport:
            logger.warning("[PACK] Transport port unavailable; cannot spawn")
            return

        parent_meta = self._transport.get_connection_meta(parent_chat_id)
        if parent_meta is None:
            logger.debug("[PACK] No connection for parent chat=%s", parent_chat_id)
            return
        app_id = parent_meta["app_id"]
        user_id = parent_meta["user_id"]
        ws_id = parent_meta["ws_id"]
        ...
        # --- Pause parent ---
        try:
            await self._transport.pause_workflow(parent_chat_id, "spawn_children")
        ...
        pm = self._transport.get_persistence()

        for st in spawnable:
            target_workflow_name = st.workflow_name
            child_workflow_name = target_workflow_name if spawn_mode == "workflow" else str(shared_workflow)
            ...
            if not initial_agent_override:
                ...
                if spawn_mode == "shared_workflow" and isinstance(cfg_initial_agent, str) and cfg_initial_agent.strip():
                    initial_agent_override = cfg_initial_agent.strip()

            # Resolve initial message.
            initial_message = st.initial_message
            if not initial_message and spawn_mode == "shared_workflow":
                wf_desc = st.metadata.get("description", "")
                if wf_desc:
                    initial_message = f"Run workflow '{target_workflow_name}'. Description: {wf_desc}"
                else:
                    initial_message = f"Run workflow '{target_workflow_name}'."

            # Generate chat_id.
            if spawn_mode == "shared_workflow":
                new_chat_id = f"chat_shared_{target_workflow_name}_{uuid.uuid4().hex[:8]}"
            else:
            ...
            if spawn_mode == "shared_workflow":
                # Seed child context from sub-task metadata (generic — no
                # platform-specific keys like PatternSelection/InterviewTranscript).
                ...
                extra_fields.update(
                    {
                        ...
                        "target_workflow_description": st.metadata.get("description"),
                        ...
                        "parent_transcript": initial_message,
                    }
                )
                # Merge static child_context_seed from trigger config.
                child_seed = trigger_entry.get("child_context_seed")
                if isinstance(child_seed, dict):
                    extra_fields.update(child_seed)
            ...
            await self._transport.spawn_run(_child_req)
        ...
        # --- Notify UI ---
        try:
            await self._transport.send_ui_event(
                {
    ...
    async def _handle_fan_in_completion(self, chat_id: str, parent_chat_id: str) -> None:
        ...
        for child_id in list(active.child_chat_ids):
            if self._transport.is_task_running(child_id):
                all_done = False
            else:
                done_count += 1
    ...
    async def _handle_journey_auto_advance(self, payload: Dict[str, Any], chat_id: str) -> None:
        ...
        if not self._transport:
            return
        meta = self._transport.get_connection_meta(chat_id)
        if meta:
            workflow_name = workflow_name or str(meta.get("workflow_name") or "").strip()
            app_id = app_id or str(meta.get("app_id") or "").strip()
    ...
    async def _journey_advance_inner(
        ...
        *,
        ...
        from mozaiksai.runtime.data.models import WorkflowStatus as _WS
        ...
        for wf in next_group:
            ...
            await self._transport.setup_child_connection(
            ...
            await self._transport.flush_pre_connection_buffers(next_chat_id)
        ...
        for wf, cid, _ in spawned:
            try:
                ...
                await self._transport.spawn_run(_journey_req)
            except Exception:
                continue
    ...
    async def _timeout_watchdog(self, parent_chat_id: str, timeout_seconds: float) -> None:
        ...
        await self._handle_partial_failure(active)
    ...
    async def _handle_partial_failure(
        ...
        if strategy == PartialFailureStrategy.FAIL_ALL:
            ...
            for child_id in list(active.child_chat_ids):
                try:
                    await self._transport.pause_workflow(
                        chat_id=child_id, reason="timeout_fail_all"
                    )
                except Exception:
                    pass
            # Cleanup and resume with failure info.
            await self._finalize_with_available(active, timed_out=True)
                ...
                await self._transport.send_ui_event(
                    {
                        ...
                        "data": {
                            ...
                            "children_pending": [
                                ...
                                if self._transport.is_task_running(cid)
                            ],
                        },
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                    },
    ...
    async def recover_from_persistence(self) -> int:
        ...
        Call this once when the coordinator is first instantiated (e.g., during
        transport startup).  It:
    ...
    async def _collect_child_results(
        ...
        for child_id in active.child_chat_ids:
            ...
            if self._transport:
                err = self._transport.get_task_error(child_id)
                if err is not None:
                    success = False
                    error_msg = err
    ...
    async def _resume_parent(
        ...
        *,
        ...
        try:
            from mozaiksai.kernel.orchestration_events import emit_parent_resuming
        ...
        try:
            await self._transport.pause_workflow(chat_id=parent_chat_id, reason="resume_restart")
        except Exception:
            pass
        ...
        _resume_req = _RunRequest(
            ...
            capability="resume_parent",
            metadata={"resume_agent": resume_agent},
        )
        await self._transport.spawn_run(_resume_req)
...
# ---------------------------------------------------------------------------
# Built-in: collect_all merge (raw child_extra dump)
# ---------------------------------------------------------------------------

class _CollectAllMerge:
    """Merge strategy: dumps raw child ``extra_context`` keyed by task_id."""
