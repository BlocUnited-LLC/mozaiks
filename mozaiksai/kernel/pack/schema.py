...
class SpawnMode(str, Enum):
    ...
    - shared_workflow:   All sub-tasks share one workflow definition
                         but run with different prompts / context seeds.
    """
    WORKFLOW = "workflow"
...
class MFJFanOutConfig(BaseModel):
    ...
    Fields:
        spawn_mode:            "workflow" or "shared_workflow".
        shared_workflow:        Required when spawn_mode is "shared_workflow".
    ...
    shared_workflow: Optional[str] = None
    ...
    @field_validator("shared_workflow")
    @classmethod
    def _shared_workflow_required(cls, v: Optional[str], info) -> Optional[str]:
        """Validate shared_workflow is set when spawn_mode is shared_workflow."""
...
class PerWorkflowPackGraph(BaseModel):
    ...
    Supports two key layouts with automatic version detection:
    ...
    def triggers(self) -> List[MidFlightJourney]:
        ...
        # v2 — normalize raw dicts into typed objects
...
def _v2_entry_to_mfj(entry: Dict[str, Any]) -> MidFlightJourney:
    ...
    Maps flat v2 fields to the nested fan_out/fan_in structure:
        ...
        shared_workflow            → fan_out.shared_workflow
    ...
    gen_wf = str(entry.get("shared_workflow") or "").strip()
    if gen_wf:
        fan_out_kwargs["shared_workflow"] = gen_wf
...
def _mfj_to_v2_dict(mfj: MidFlightJourney) -> Dict[str, Any]:
    ...
    Produces the legacy-flat representation for code that still uses
    ``.get()`` patterns on journey dicts.
    """
    d: Dict[str, Any] = {
        "id": mfj.id,
    ...
    if fo.shared_workflow:
        d["shared_workflow"] = fo.shared_workflow
...
def parse_pack_graph(data: Dict[str, Any]) -> PerWorkflowPackGraph:
    ...
    Handles v2 (journeys) and v3 (mid_flight_journeys) automatically.
    Invalid entries are logged and skipped — the parser is lenient to
    avoid breaking on partial configs.
