...
from mozaiksai.runtime.config import get_mongo_client, get_mongo_db_name
from mozaiksai.runtime.multitenant import build_app_scope_filter, coalesce_app_id, dual_write_app_scope
from ..models import WorkflowStatus

# Lazy imports for streaming event types to avoid load-time engine dependency.
# The persistence layer accepts engine-internal DomainEvent objects from orchestration
# but delays the import to avoid coupling at module load time.
_InternalDomainEvent = None
_EventKind = None


def _get_streaming_types():
    """Lazy loader for streaming event types."""
    global _InternalDomainEvent, _EventKind
    if _InternalDomainEvent is None:
        from mozaiksai.engine.streaming.domain_event import DomainEvent, EventKind
        _InternalDomainEvent = DomainEvent
        _EventKind = EventKind
    return _InternalDomainEvent, _EventKind
...
class PersistenceManager:
    ...
    async def _ensure_client(self) -> None:
        ...
        async with self._init_lock:
            ...
            _db_name = get_mongo_db_name()
            try:
                # Primary chat session collection (canonical)
                coll = self.client[_db_name]["ChatSessions"]
                ...
                general_coll = self.client[_db_name][_GENERAL_CHAT_COLLECTION]
                ...
                counter_coll = self.client[_db_name][_GENERAL_CHAT_COUNTER_COLLECTION]
...
class AG2PersistenceManager:
    ...
    async def _coll(self):
        ...
        return self.persistence.client[get_mongo_db_name()]["ChatSessions"]

    async def _workflow_stats_coll(self):
        await self.persistence._ensure_client()
        assert self.persistence.client is not None, "Mongo client not initialized"
        return self.persistence.client[get_mongo_db_name()]["WorkflowStats"]

    async def _general_coll(self):
        await self.persistence._ensure_client()
        assert self.persistence.client is not None, "Mongo client not initialized"
        return self.persistence.client[get_mongo_db_name()][_GENERAL_CHAT_COLLECTION]

    async def _general_counter_coll(self):
        await self.persistence._ensure_client()
        assert self.persistence.client is not None, "Mongo client not initialized"
        return self.persistence.client[get_mongo_db_name()][_GENERAL_CHAT_COUNTER_COLLECTION]
    ...
    # Events ------------------------------------------------------------
    async def save_event(self, event: Any, chat_id: str, app_id: Optional[str] = None) -> None:
        """Persist a streaming DomainEvent to the chat session.

        Parameters
        ----------
        event : DomainEvent
            A streaming DomainEvent from engine.streaming.domain_event.
            Only TEXT events are persisted; other event kinds are ignored.
        chat_id : str
        ...
        if not resolved_app_id:
            raise ValueError("app_id is required")
        try:
            # Lazy load streaming types to avoid load-time engine dependency
            DomainEventType, EventKindType = _get_streaming_types()

            # Only persist agent.message events; ignore all other event kinds
            if not isinstance(event, DomainEventType) or event.kind != EventKindType.TEXT:
                return
            coll = await self._coll()
            ...
            try:
                ...
                seq = 1
                wf_name = None
            event_id = event.metadata.get("event_id") or event.metadata.get("event_uuid") or str(uuid4())
            raw_name = event.agent
            raw_content = event.content if event.content is not None else ""
            ...
            ts = event.metadata.get("timestamp") if isinstance(event, DomainEventType) else getattr(event, "timestamp", None)
