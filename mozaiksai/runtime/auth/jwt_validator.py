...
from mozaiksai.runtime.auth.config import get_auth_config, AuthConfig
from mozaiksai.runtime.auth.jwks import get_jwks_client
...
class TokenClaims:
    ...
    def validate_app_id(self, path_app_id: str) -> bool:
        ...
        if not self.mozaiks_app_id:
            # No app_id claim in token — not scoped to a specific app
            return True
...
class JWTValidator:
    ...
    async def _get_issuer(self) -> str:
        ...
        if self._config.use_discovery:
            from mozaiksai.runtime.auth.discovery import get_discovery_client
    ...
    async def validate_token(
        ...
        # Scopes: check "scp" (Azure AD) then "scope" (Keycloak / standard OIDC)
