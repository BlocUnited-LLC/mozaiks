...
Bundle keys (all optional):
    ...
    register_workers      (*, worker_registry: WorkerRegistry,
                             capability_registry: CapabilityRegistry) -> None
        Register additional capability workers into the runtime.
        Called during startup after AgentWorker is registered, so platform
        layers can add workers for non-agent capabilities (e.g. 'provision',
        'build') without modifying factory.py.
...
_BUNDLE_KEYS = ("on_startup", "register_workers", "chat_prereqs", "chat_session_fields", "workflow_ordering")
...
class PlatformHookRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._register_workers_hooks: List[Callable] = []
    ...
    def _register_bundle(self, bundle: Any, source: str = "") -> None:
        ...
        slot_map = {
            ...
            "register_workers": self._register_workers_hooks,
    ...
    def call_register_workers(
        self,
        worker_registry: Any,
        capability_registry: Any,
    ) -> None:
        """Invoke all register_workers hooks.

        Called from ``factory.py`` startup after AgentWorker is registered.
        Platform layers add workers for non-agent capabilities here
        (e.g. capability='provision', capability='build').
        """
        for hook in self._register_workers_hooks:
            try:
                hook(
                ...
                logger.warning(f"PLATFORM_HOOKS_REGISTER_WORKERS_ERROR: {exc}")

        if self._register_workers_hooks:
            logger.info(
                f"PLATFORM_HOOKS: ran {len(self._register_workers_hooks)} register_workers hook(s)"
            )
    ...
    @property
    def has_register_workers(self) -> bool:
        return bool(self._register_workers_hooks)

    def summary(self) -> Dict[str, Any]:
        return {
            "startup_hooks": len(self._startup_hooks),
            "register_workers_hooks": len(self._register_workers_hooks),
