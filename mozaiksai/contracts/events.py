...
class DomainEvent(BaseModel):
    ...
    @classmethod
    def _coerce_data(cls, raw: Any) -> Any:
        """Accept legacy ``payload`` key from older construction sites and persisted data."""
        if isinstance(raw, dict) and "data" not in raw and "payload" in raw:
            updated = dict(raw)
            updated["data"] = updated.pop("payload")
            return updated
        return raw
...
__all__ = ["DomainEvent", "EVENT_SCHEMA_VERSION", "EVENT_TYPE_PATTERN", "EventEnvelope"]
