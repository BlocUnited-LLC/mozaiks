# ==============================================================================
# FILE: resume.py
# DESCRIPTION: Session resume and message replay for WebSocket transports.
# ==============================================================================

"""Session resume and history replay for WebSocket transports.

This module handles:
- Auto-resume on reconnect (replaying persisted messages to restore UI state)
- Explicit client.resume requests (gap-filling after network interruption)
- Boundary event emission to signal replay completion

The resumer is engine-agnostic — it works with persisted message history
regardless of the underlying execution engine (AG2, pipelines, etc.).
"""
...
class SessionResumer:
    """Handles session resume and message replay for WebSocket transports.

    Responsibilities:
    - Replay persisted messages when a client reconnects mid-session
    - Handle explicit resume requests from clients
    - Emit boundary events to signal replay completion
    - Filter seed/hidden messages based on workflow configuration
    """

    def __init__(self) -> None:
        self.logger = get_core_logger("session_resumer")
    ...
    async def _replay_messages(
        ...
        *,
        ...
        for offset, message in enumerate(slice_messages):
            ...
            # Filter out seed messages (hidden kickstart messages) based on workflow config.
            # Workflows with startup_mode="AgentDriven" or hide_seed_messages=True
            # should not replay seed messages to the UI on reconnect.
            should_skip = False
            hide_seed = startup_mode in ("AgentDriven", "agent_driven") or startup_mode == "hide_seeds"
            if hide_seed:
                # Check if this is a seed message (hidden orchestration kickstart)
                ...
                if seed_kind == "initial_message" or metadata_seed_kind == "initial_message":
                    self.logger.debug(
                        "[SESSION_RESUME] Skipping seed message (index=%d, chat_id=%s)",
    ...
    def _build_boundary_event(
        ...
        *,
        ...
        # Build resume state (engine-agnostic replay context)
        resume_state = {
            "mode": mode,
            "start_index": start_index,
            "last_index": last_index,
        ...
        if events_slice:
            resume_state["last_speaker_name"] = events_slice[-1].get("name") or events_slice[-1].get("agent_name")
