...
async def _wait_for_ui_tool_response_internal(event_id: str, timeout: Optional[float] = None) -> Dict[str, Any]:
    from mozaiksai.transport.websocket.handler import SimpleTransport  # local import
    transport = await SimpleTransport.get_instance()
    try:
...
async def use_ui_tool(
    ...
    *,
    ...
    The `display` parameter is now optional and will be auto-resolved from the
    workflow tools configuration if not provided. This eliminates redundancy
    between tool declarations and tool function implementations.
    """
    wf_logger = get_workflow_logger(workflow_name=workflow_name, chat_id=chat_id)
    start = _dt.datetime.now(_dt.timezone.utc)
    
    # Get the ACTUAL agent owner from the tools config (not from conversation context)
    agent_name = None
    try:
    ...
    if chat_id:
        try:
            from mozaiksai.runtime.data.persistence import persistence_manager as pm
            from mozaiksai.runtime.config import get_app_id_from_chat_or_context
