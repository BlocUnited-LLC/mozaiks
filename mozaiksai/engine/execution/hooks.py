...
This module reads a workflow's hooks configuration (from the cached YAML
config) and registers the declared hook functions on the appropriate
`ConversableAgent` instances.

HOOKS YAML FORMAT (each entry):
...
3. Default function name: if only module path given with no explicit function
   we look for a top-level symbol named exactly as provided in `function`.
...
def register_hooks_for_workflow(
    workflow_name: str,
    agents: Dict[str, Any],
    *,
    ...
    """Register AG2 hooks on agents for a workflow.

    Resolution order for hook declarations:

    1. ``hooks_config`` dict (pre-loaded from ``hooks.yaml`` by workflow_manager)
    2. ``hooks.yaml`` on disk (``config/`` subdir or workflow root)
    ...
    hooks_config: Pre-loaded hooks config dict (from workflow_manager).
        Expected shape: ``{"hooks": [<entry>, ...]}`` **or** just ``[<entry>, ...]``.
    ...
    # --- Resolve hook entries from the best available source ---
    entries: Optional[list] = None

    # 1. Pre-loaded config dict (from hooks.yaml via workflow_manager)
    if hooks_config is not None:
        if isinstance(hooks_config, dict):
            entries = hooks_config.get("hooks") or hooks_config.get("hook_rules")
            # If the dict itself is a list-of-hooks wrapper, try top-level
            if entries is None and isinstance(hooks_config.get(next(iter(hooks_config), "")), list):
                entries = next(iter(hooks_config.values()), None)
        elif isinstance(hooks_config, list):
            entries = hooks_config
        if entries is not None:
            logger.info(f"Loading hooks for workflow '{workflow_name}' from pre-loaded config (hooks.yaml)")

    # 2. YAML file on disk (config/ subdir or root)
    if entries is None:
        import yaml as _yaml

        for candidate in (
            workflow_path / "config" / "hooks.yaml",
            workflow_path / "hooks.yaml",
        ):
            if candidate.exists():
                try:
                    ...
                    if isinstance(yaml_data, dict):
                        entries = yaml_data.get("hooks") or yaml_data.get("hook_rules")
                    elif isinstance(yaml_data, list):
                        entries = yaml_data
                    if entries is not None:
                        logger.info(f"Loading hooks for workflow '{workflow_name}' from: {candidate}")
                        break
                except Exception as yaml_err:
                    logger.error(f"Failed reading hooks YAML {candidate} for {workflow_name}: {yaml_err}")

    if entries is None or not isinstance(entries, list):
        logger.debug(f"No hook declarations found for workflow {workflow_name}")
        return []
...
def summarize_hooks(workflow_name: str, *, hooks_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    """Return raw hook declarations (without importing) for inspection.

    Uses pre-loaded config when available; avoids independent disk reads.
    """
    if hooks_config is not None:
        if isinstance(hooks_config, dict):
            entries = hooks_config.get("hooks") or hooks_config.get("hook_rules")
    ...
    # Only reach disk when called outside normal workflow lifecycle (e.g. debug CLI)
    from mozaiksai.kernel.workflow_manager import workflow_manager
    cached = (workflow_manager.get_config(workflow_name) or {}).get('hooks')
    if cached:
        return summarize_hooks(workflow_name, hooks_config=cached)
