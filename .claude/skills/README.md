...
| Control-plane / refinement / harness routing | `control-plane-refinement-change` | Covers `app/config/ai.json` startup, `app/config/refinement_policy.yaml`, `refinement_harness/config/harness.yaml`, artifact routing, and checkpoint re-entry. Add `factory-build-workflow-change` too when `workflow_sequence` composition or `extension_registry.json` routing changes. |
