...
- module, page, or database intent generation behavior
...
5. Database intent generation changes:
   - inspect `structured_outputs.yaml`, `file_contracts.yaml`, and persistence tests together
   - keep `config/database_intent.json` and `config/database_migrations/{migration_id}.json` as the canonical output family
...
- database intent and persistence alignment:
