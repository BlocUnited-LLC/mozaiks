...
async def persist_writers_summary(*, context_variables: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        set_direction = str(context_variables.get("set_direction") or "").strip().lower()
        set_brief_packet = context_variables.get("set_brief_packet")
        writers_summary = context_variables.get("writers_summary")
    except Exception:
        return {"status": "skipped", "reason": "context_read_failed"}

    if not chat_id or not app_id or not isinstance(writers_summary, dict):
    ...
    update = {
        ...
        "audience": audience,
        "tone": tone,
        "set_brief_packet": set_brief_packet if isinstance(set_brief_packet, dict) else {},
        "writers_summary": writers_summary,
        "set_direction": set_direction or writers_summary.get("recommended_direction"),
