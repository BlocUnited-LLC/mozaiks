    ...
    if isinstance(plan, dict):
        for pack in plan.get("capability_packs") or []:
            if isinstance(pack, dict):
                ...
                if pack_type in _AI_NATIVE_PACKS and pack_type not in detected:
...
def _pack_instructions(pack_id: str) -> str:
    ...
    if pack_id == "ai_review_pack":
        return (
            ...
            "7. The surface_map entry for this module must declare:\n"
            "   workflow_triggers: [\"{module_id}-review-workflow\"]\n"
            "8. The AI review workflow bundle is generated in a separate AgentGenerator run "
    ...
    if pack_id == "ai_analysis_pack":
        return (
            ...
            "8. The AI analysis workflow bundle is generated in a separate AgentGenerator run "
    ...
    if pack_id == "ai_extraction_pack":
        return (
            ...
            "8. The AI extraction workflow bundle is generated in a separate AgentGenerator run "
            "using the ai_extraction archetype, capability_id={module_id}-extraction-workflow, "
            "startup_mode=BackendOnly with task_batches for parallel workers.\n"
        )
...
def _build_body(detected_packs: list[str]) -> str:
    parts: list[str] = [
        ...
        "events, reactions.yaml wiring, event_flows declarations, AND "
        "workflow_triggers on the surface_map entry so AgentGenerator knows "
        "which capability_ids to generate.\n",
        ...
        "Use this convention in reactions.yaml, event_flows, AND surface_map.workflow_triggers "
        "so the platform routes domain events to the correct AgentGenerator workflow.\n",
    ]
