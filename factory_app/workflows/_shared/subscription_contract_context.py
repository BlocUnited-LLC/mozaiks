...
def _render_contract(contract: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "If this contract's subscription_config_file declares assignment_store and the app does not use the mozaikspay managed_capability, AppPlanAgent must also include an entitlement_dispatch module task set: one module_contract task (owned_paths: ['modules/entitlement_dispatch/module.yaml'], initial_agent: 'ConfigMiddlewareAgent') and one business_services task (owned_paths: ['modules/entitlement_dispatch/backend/handler.py', 'modules/entitlement_dispatch/backend/service.py', 'modules/entitlement_dispatch/backend/repo.py'], initial_agent: 'ServiceAgent'). That module is the write-side partner for ConfiguredEntitlementAdapter (the runtime read side). The bundle scanner rejects apps that declare assignment_store without this module.",
            body,
        ]
    )
