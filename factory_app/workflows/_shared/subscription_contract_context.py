...
def _render_contract(contract: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "Since contract_required is true, AppPlanAgent MUST include a build task with task_type='subscription_config', capability_pack_id=null, surface_id='subscription_contract', surface_kind='control_plane', initial_agent='ConfigMiddlewareAgent', owned_paths=['config/subscriptions.yaml']. That task serializes this contract's subscription_config_file.",
            "If this contract's subscription_config_file declares assignment_store and the app does not use the mozaikspay managed_capability, AppPlanAgent must also include three entitlement_dispatch tasks: (1) one data_migrations task (capability_pack_id: 'entitlement_dispatch', owned_paths: ['data/migrations/001_entitlement_dispatch_collections.json'], initial_agent: 'DatabaseAgent') that declares the billing.subscriptions data alias, (2) one module_contract task (capability_pack_id: 'entitlement_dispatch', owned_paths: ['modules/entitlement_dispatch/module.yaml'], initial_agent: 'ConfigMiddlewareAgent'), and (3) one business_services task (capability_pack_id: 'entitlement_dispatch', owned_paths: ['modules/entitlement_dispatch/backend/handler.py', 'modules/entitlement_dispatch/backend/service.py', 'modules/entitlement_dispatch/backend/repo.py'], initial_agent: 'ServiceAgent'). The data_migrations task must run before the business_services task because repo.py reads from the billing.subscriptions alias declared by that migration. That module is the write-side partner for ConfiguredEntitlementAdapter (the runtime read side). The bundle scanner rejects apps that declare assignment_store without this module.",
            body,
        ]
    )
