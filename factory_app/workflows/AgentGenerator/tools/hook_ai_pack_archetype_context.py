...
write results back through the callback module action.
...
def _build_callback_body(ai_surfaces: list[dict]) -> str:
    lines: list[str] = [
        ...
        "REQUIRED for AI-native pack workflows:\n"
        "\n"
    ...
    if ai_surfaces:
        ...
        lines.append(
            ...
            "the exact module_id and result_action for the callback endpoint.\n"
        )
...
def inject_ai_pack_archetype_context(
    ...
    worker generating an AI-native pack workflow knows to declare
    backend_request and write results back through the callback action.
