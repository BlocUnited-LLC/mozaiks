...
#              If connector exists and is active, returns the key from Key Vault.
#              If expired or missing, prompts user via UI tool.
# ==============================================================================
...
async def get_or_request_api_key(
    ...
    2. If YES: fetch secret from Key Vault, return for E2B use
    3. If NO/EXPIRED: prompt user via UI, store new connector
    ...
    try:
        ...
        if status["exists"] and status["status"] in ("active", "expiring"):
            ...
            # Fetch actual secret from Key Vault
            ...
            if secret_result["success"]:
                ...
                # Key Vault fetch failed - need to re-request
    ...
    try:
        from workflows.AgentGenerator.tools.request_api_key import request_api_key
        
        ui_result = await request_api_key(
            service=service_norm,
            display_name=display,
            store_connector=True,  # Store in Key Vault + AppConnectors
