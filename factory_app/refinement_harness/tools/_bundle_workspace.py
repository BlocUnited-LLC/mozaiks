...
async def load_bundle_workspace(
    *,
    record_store: BuildRecordStore,
    app_id: str,
    build_record_id: str,
    content_store: BundleContentStore | None = None,
) -> dict[str, Any]:
    artifact = await record_store.get_build_record(
        app_id=app_id,
    ...
    if workspace_dir and Path(str(workspace_dir)).exists():
        ...
        file_map = read_bundle_zip(Path(str(bundle_path)))
        source = "artifact_zip"
    elif content_ref:
...
def read_workspace_dir(root: Path) -> dict[str, str]:
    ...
    for current, dir_names, file_names in os.walk(root):
        ...
        for file_name in sorted(file_names):
            ...
            if rel is None:
                continue
            if _skip_bundle_workspace_path(rel):
                continue
...
def _read_from_archive(archive: zipfile.ZipFile) -> dict[str, str]:
    ...
    for info in sorted(archive.infolist(), key=lambda item: item.filename):
        ...
        if safe_name is None:
            continue
        if _skip_bundle_workspace_path(safe_name):
            continue
        try:
...
def _skip_bundle_workspace_path(path: str) -> bool:
...
def read_bundle_zip(zip_path: Path) -> dict[str, str]:
    with zipfile.ZipFile(zip_path, "r") as archive:
        return _read_from_archive(archive)


def read_bundle_zip_bytes(data: bytes) -> dict[str, str]:
    """Read a bundle zip from raw bytes (e.g. retrieved from a content store)."""
