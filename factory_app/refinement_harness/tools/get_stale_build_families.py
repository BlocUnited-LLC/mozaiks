...
from mozaiksai.core.artifacts import BuildRecordStore, get_build_record_store

from ._shared import normalize_context


async def get_stale_build_families(
    *,
    context: ControlPlaneToolContext | dict[str, Any] | None = None,
    record_store: BuildRecordStore | None = None,
) -> dict[str, Any]:
    """Return the build families that have stale versions for this app.

    A family is stale when a prior build sequence invalidated it — meaning a
    dependent build record was written but this family was not yet refreshed. The
...
__all__ = ["get_stale_build_families"]
