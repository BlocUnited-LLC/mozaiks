...
Coding-agent guidance for `MozaiksFactory`.
...
- Provider-neutral deployment artifacts such as `Dockerfile`, `docker-compose.yml`, `.env.example`, `.env.staging.example`, `.env.production.example`, `.github/workflows/readiness.yml`, `.github/workflows/deploy.yml`, and `deployment.manifest.json` live at the app bundle root when generated; they are emitted by the OSS deployment contract renderer, not by `app/services/`.
