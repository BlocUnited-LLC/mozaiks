...
def _default_workspace_host_inputs() -> Dict[str, Any]:
    workspace = _workspace_root()
    sibling_root = workspace.parent

    frontend_repo = sibling_root / "MOZ-UI"
    backend_repo = sibling_root / "mozaiks-platform" / "services"

    data: Dict[str, Any] = {
        "frontend_repo_path": str(frontend_repo) if frontend_repo.exists() else None,
        "backend_repo_path": str(backend_repo) if backend_repo.exists() else None,
        "discovery_mode": "guided",
    }

    env_backend_base = os.getenv("MOZAIKS_DOGFOOD_BACKEND_BASE_URL") or os.getenv("EXISTING_APP_BACKEND_BASE_URL")
    env_openapi_url = os.getenv("MOZAIKS_DOGFOOD_OPENAPI_URL") or os.getenv("EXISTING_APP_OPENAPI_URL")
    if env_backend_base:
...
def _resolve_host_app_source_inputs(host_app_source: Optional[str]) -> Dict[str, Any]:
    ...
    if not source:
        return {}
    if source == "workspace_host":
        return _default_workspace_host_inputs()
    return {}
...
async def collect_prechat_discovery_context(context_variables: Optional[Any] = None) -> Dict[str, Any]:
    ...
    if repo_summary.get("success") and host_app_source != "workspace_host" and not _ctx_get(ctx, "app_name"):
    ...
    if api_inventory.get("success"):
        ...
        if host_app_source != "workspace_host" and not _ctx_get(ctx, "app_name") and api_inventory.get("title"):
            _ctx_set(ctx, "app_name", api_inventory["title"])

    if host_app_source == "workspace_host":
        if not _ctx_get(ctx, "app_name"):
            _ctx_set(ctx, "app_name", "Mozaiks")
        if not _ctx_get(ctx, "app_description"):
            _ctx_set(
                ctx,
                "app_description",
                "Existing Mozaiks product composed of the MOZ-UI frontend and mozaiks-platform services backend.",
            )
        if not _ctx_get(ctx, "existing_experience_summary"):
            _ctx_set(
                ctx,
                "existing_experience_summary",
                "Current host experience is delivered by the MOZ-UI frontend while backend capabilities live in the mozaiks-platform services repo.",
            )
