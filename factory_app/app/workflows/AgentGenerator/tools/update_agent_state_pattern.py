...
def inject_workflow_implementation_guidance(agent, messages: List[Dict[str, Any]]) -> None:
    ...
    NOTE: `integration` inside ModuleAgents is planning metadata only. It helps
    ...
    try:
        ...
        matrix_rules = """
[PLANNING CONTRACT]
`integration` fields in ModuleAgents are planning hints only.
  ...
  "tools": [
    ...
    },
    {
      ...
      "installRequirements": ["google-genai"]
        ...
        planning_note = (
            "[PLANNING CONTRACT]\n"
            "- `integration` values in ModuleAgents are planning hints only for dependency-aware code generation.\n"
...
def inject_ui_file_generator_guidance(agent, messages: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        contract_rules = (
            ...
            "- Do not call low-level `send_ui_tool_event` directly, or subscribe via `useAppEventBus` in generated React.\\n"
            "- Generated React components are mounted by `WorkflowUIRouter` and receive props like `payload`, `onResponse`, `onCancel`, `ui_tool_id`, `eventId`, `workflowName`, and `componentId`.\\n"
